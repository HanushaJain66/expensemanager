...
function YearlyIncome({inTotal, userId, result }) {
    ...
    useEffect(() => {
        fetch('/api', {
            ...
            body: JSON.stringify("SELECT I.TRANSICTION_ID,I.TITLE, I.AMOUNT, D.ACCOUNT_NAME, I.CATEGORY, U.USER_NAME,to_char(EXTRACT(MONTH FROM I.TRANSICTION_DATE)) FROM DEFAULT_INCOME I  JOIN DEFAULT_ACCOUNT D ON I.ACCOUNT_ID = D.ACCOUNT_ID AND I.USER_ID = D.USER_ID  LEFT JOIN USER_DATA U ON I.USER_ID = U.USER_ID  WHERE to_char(EXTRACT(YEAR FROM I.TRANSICTION_DATE)) = '2023' AND I.USER_ID = '" + userId + "' ORDER BY to_char(EXTRACT(MONTH FROM I.TRANSICTION_DATE))")
        })
    ...
    return (
        <div id='yearly-income'>
            <div id='income-title'>Total Income this year as per Month<span id='in-total'>{'₹ ' + inTotal}</span></div>
